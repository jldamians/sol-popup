...
class PopupSOL {
  ...
  async close() {
    ...
    if (campaign && campaign.exist === true) {
      ...
      await this._page.waitFor((campaigns=[], selector=null) => {
        /*let closed = true;

        campaigns.forEach((campaign) => {
          closed = closed === true && campaign.exist === false;
        });*/

        const popup = document.querySelector(selector);

        return /*closed === true && */popup.className === 'oculto';
      }, { timeout: 11 * 1000 }, this._campaigns, 'div#divModalCampana');
  ...
  async _closeInformativeDialog() {
    ...
    try {
      ...
      const closeBtn = await this._frame.waitForSelector(selector, { timeout: 5 * 1000, visible: true });
  ...
  async _closeConfirmDialog() {
    ...
    //    [*] 20191105 - Img 03 - cerrar confirmacion telefono.PNG
    //  # Casos de uso
    //    [*] Confirmación su teléfono celular
    // Cerramos el Pop-Up de confirmación
    try {
      const selector = '#continuarSinRegistrarBtn';

      const closeBtn = await this._frame.waitForSelector(selector, { timeout: 5 * 1000, visible: true })

      await closeBtn.click();
    } catch (exception) {
      throw new Error('No se ha logrado cerrar el mensaje de confirmación');
    }
  }
}
