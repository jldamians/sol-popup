...
class PopupSOL {
        ...
        if (successfulRequest === true) {
          ...
          if (request.postData() == 'action=campana') {
            ...
            let campaign = campaigns.find((campaign) => {
              const allowed = ['ModificaDatosRuc', 'CampanhaPerfil'];
  ...
  async close() {
    ...
    if (!!this._campaign) {
      ...
      */
      ...
      const closablePopup = (
        this._campaign.name === 'ModificaDatosRuc' && this._campaign.url.indexOf('/campmodificadatosruc.htm') !== -1
      );
      ...
      const nonClosablePopup = (
        this._campaign.name === 'ModificaDatosRuc' && this._campaign.url.indexOf('/campanhas.htm') !== -1
      );

      const closeInformativeOnly = this._campaign.name === 'CampanhaPerfil';

      if (closablePopup === true || closeInformativeOnly === true) {
        try {
  ...
  async _closeInformativeDialog() {
    ...
    const screenshot = await HTMLElement.screenshot({ path: 'informative-dialog.png' });
  ...
  async _closeConfirmDialog() {
    ...
    //    [*] Campaña de indicadores de cumplimiento tributario
