...
class PopupSOL {
  ...
  async close() {
    ...
    if (!!this._campaign && this._campaign.exist === true) {
      ...
      */
      ...
      const closablePopup = this._campaign.url.indexOf('/campmodificadatosruc.htm') !== -1;

      // Cuando la campaña de modificación de datos lanza un
      // request ha "/ol-ti-itmoddatosruc/campanhas.htm",
      // la acción de cerrar el popup terminará la sesión
      const nonClosablePopup = this._campaign.url.indexOf('/campanhas.htm') !== -1;
